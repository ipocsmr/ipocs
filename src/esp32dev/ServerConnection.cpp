...
esp::ServerConnection::ServerConnection() {
    ...
    IPAddress ip(172,16,0,6);
    this->setServer(ip, 10000);
}
...
void esp::ServerConnection::loop(bool isWiFiConnected) {
    ...
    //}
    if (!this->tcp->connected()) {
        /*int nrOfServices = MDNS.queryService("ipocs", "tcp");
        if (nrOfServices != 0) {
            ...
            for (int i = 0; i < nrOfServices; i = i + 1)
            {
                ...
                esp::ServerConnection::instance().setServer(ip, MDNS.port(i));
            }
        }
        */
        ...
        while (this->tcp->available()) {
            ...
            // Now parse it.
            char outStr[255*3];
            array_to_string(message, RL_MESSAGE, outStr);
            Serial.printf("%s:%i %s\n", __FILE__, __LINE__, outStr);
...
void esp::ServerConnection::send(IPOCS::Message* msg) {
    ...
    char outStr[255*3];
    array_to_string(buffer, len, outStr);
    Serial.printf("%s:%i %s\n", __FILE__, __LINE__, outStr);
}